...
public class RunDisasterRecoveryClusterCmd extends BaseCmd {
    ...
    public void execute() throws ServerApiException {
        try {
            boolean result = disasterRecovery.runDisasterRecoveryCommand(this);
            if(result) {
                SuccessResponse response = new SuccessResponse(getCommandName());
                response.setSuccess(result);
                this.setResponseObject(response);
            } else {
                throw new ServerApiException(ApiErrorCode.INTERNAL_ERROR, "Failed to disaster recovery for management server. Please check the Security Check tab for detailed results.");
            }
