...
@APICommand(name = PromoteDisasterRecoveryClusterCmd.APINAME,
        description = "promote a disaster recovery cluster",
        responseObject = SuccessResponse.class,
        entityType = {DisasterRecoveryCluster.class},
        authorized = {RoleType.Admin})
public class PromoteDisasterRecoveryClusterCmd extends BaseAsyncCmd {
    public static final String APINAME = "promoteDisasterRecoveryCluster";
    ...
    public String getEventType() {
        return DisasterRecoveryClusterEventTypes.EVENT_DR_PROMOTE;
    }

    @Override
    public String getEventDescription() {
        String description = "Promoting disaster recovery cluster";
    ...
    public void execute() throws ServerApiException {
        try {
            if (!disasterRecoveryClusterService.promoteDisasterRecoveryCluster(this)) {
                throw new ServerApiException(ApiErrorCode.INTERNAL_ERROR, String.format("Failed to promote disaster recovery cluster ID: %d", getId()));
            }
