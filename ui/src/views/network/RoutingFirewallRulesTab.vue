...
<template>
  <div>
    ...
    <a-divider/>
    <a-button
      v-if="(('deleteRoutingFirewallRule' in $store.getters.apis) && this.selectedRowKeys.length > 0)"
      type="primary"
      danger
      style="width: 100%; margin-bottom: 15px"
      @click="bulkActionConfirmation()">
      <template #icon><delete-outlined /></template>
      {{ $t('label.action.bulk.delete.routing.firewall.rules') }}
    </a-button>
    <a-table
      size="small"
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'actions'">
          <tooltip-button
            ...
            :disabled="!('deleteRoutingFirewallRule' in $store.getters.apis)"
            type="primary"
            :danger="true"
    ...
    <bulk-action-view
      ...
      action="deleteRoutingFirewallRule"
      :loading="loading"
      :message="message"
...
export default {
  ...
  data () {
    return {
      ...
      message: {
        title: this.$t('label.action.bulk.delete.routing.firewall.rules'),
        confirmMessage: this.$t('label.confirm.delete.routing.firewall.rules')
      },
      loading: true,
      routingRules: [],
      newRule: {
  ...
  methods: {
    ...
    setSelection (selection) {
      ...
      this.selectedItems = (this.routingRules.filter(function (item) {
    ...
    deleteRule (rule) {
      this.loading = true
      postAPI('deleteRoutingFirewallRule', { id: rule.id }).then(response => {
        const jobId = response.deleteroutingfirewallruleresponse.jobid
        ...
        this.$pollJob({
          ...
          errorMessage: this.$t('message.remove.routing.firewall.rule.failed'),
          errorMethod: () => {
            if (this.selectedItems.length > 0) {
          ...
          loadingMessage: this.$t('message.remove.routing.firewall.rule.processing'),
    ...
    addRule () {
      ...
      this.loading = true
      postAPI('createRoutingFirewallRule', { ...this.newRule }).then(response => {
        this.$pollJob({
          jobId: response.createroutingfirewallruleresponse.jobid,
          successMessage: this.$t('message.add.routing.firewall.rule.success'),
          successMethod: () => {
            this.resetAllRules()
            this.fetchData()
          },
          errorMessage: this.$t('message.add.routing.firewall.rule.failed'),
          errorMethod: () => {
            this.resetAllRules()
            this.fetchData()
          },
          loadingMessage: this.$t('message.add.routing.firewall.rule.processing'),
          catchMessage: this.$t('error.fetching.async.job.result'),
          catchMethod: () => {
            this.resetAllRules()
            this.fetchData()
