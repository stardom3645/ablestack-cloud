...
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'AlmaLinux 9', 'KVM', 'default', 'AlmaLinux 9');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'CentOS 9', 'KVM', 'default', 'CentOS 9');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Oracle Linux 9', 'KVM', 'default', 'Oracle Linux 9');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Red Hat Enterprise Linux 9', 'KVM', 'default', 'Red Hat Enterprise Linux 9');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Rocky Linux 9', 'KVM', 'default', 'Rocky Linux 9');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'AlmaLinux 9', 'VMware', '7.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Oracle Linux 9', 'VMware', '7.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Red Hat Enterprise Linux 9', 'VMware', '7.0', 'rhel9_64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Rocky Linux 9', 'VMware', '7.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'AlmaLinux 9', 'VMware', '7.0.1.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Oracle Linux 9', 'VMware', '7.0.1.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Red Hat Enterprise Linux 9', 'VMware', '7.0.1.0', 'rhel9_64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Rocky Linux 9', 'VMware', '7.0.1.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'AlmaLinux 9', 'VMware', '7.0.2.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Oracle Linux 9', 'VMware', '7.0.2.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Red Hat Enterprise Linux 9', 'VMware', '7.0.2.0', 'rhel9_64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Rocky Linux 9', 'VMware', '7.0.2.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'AlmaLinux 9', 'VMware', '7.0.3.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Oracle Linux 9', 'VMware', '7.0.3.0', 'otherLinux64Guest');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (1, 'Red Hat Enterprise Linux 9', 'VMware', '7.0.3.0', 'rhel9_64Guest');
...
WHERE so.`default_use` = 1
  AND so.`vm_type` IN ('domainrouter','secondarystoragevm','consoleproxy','internalloadbalancervm','elasticloadbalancervm')
  AND (so.`limit_cpu_use` IS NULL OR so.`limit_cpu_use` <> 1);
...
UPDATE `cloud`.`networks` SET `public_mtu` = 1500, `private_mtu` = 1500 WHERE `removed` IS NULL AND `guest_type` = 'Isolated'  AND (COALESCE(`public_mtu`, 0) <> 1500 OR COALESCE(`private_mtu`, 0) <> 1500);
UPDATE `cloud`.`vpc` SET `public_mtu` = 1500 WHERE `removed` IS NULL AND COALESCE(`public_mtu`, 0) <> 1500;
UPDATE `cloud`.`nics` SET `mtu` = 1500 WHERE `vm_type` = 'DomainRouter' AND `removed` IS NULL AND `reserver_name` IN ('PublicNetworkGuru', 'ExternalGuestNetworkGuru') AND COALESCE(`mtu`, 0) <> 1500;
...
CREATE TABLE IF NOT EXISTS `cloud`.`resource_reservation` (
...
UPDATE `cloud`.`counter` SET `provider` = 'Netscaler' WHERE TRIM(COALESCE(`provider`, '')) = '';
...
UPDATE `cloud`.`autoscale_vmgroups` SET `state` = UPPER(`state`) WHERE `state` IS NOT NULL AND BINARY `state` <> BINARY UPPER(`state`);
...
UPDATE `cloud`.`network_offerings` no
SET no.`supports_vm_autoscaling` = 1
WHERE no.`removed` IS NULL
  AND COALESCE(no.`supports_vm_autoscaling`, 0) <> 1
  AND EXISTS (
    SELECT 1
    FROM `cloud`.`ntwk_offering_service_map` m
    WHERE m.`network_offering_id` = no.`id`
      AND m.`service` = 'Lb'
      AND m.`provider` IN ('VirtualRouter', 'VpcVirtualRouter', 'Netscaler')
  );
...
WHERE `name` = 'alert.email.addresses'
  AND COALESCE(`description`, '') <> 'Comma separated list of email addresses which are going to receive alert emails.';
...
WHERE `name` = 'secstorage.encrypt.copy'
  AND COALESCE(`description`, '') <> 'Use SSL method used to encrypt copy traffic between zones. Also ensures that the certificate assigned to the zone is used when generating links for external access.';

-- Create table to persist volume stats.
CREATE TABLE IF NOT EXISTS `cloud`.`volume_stats` (
    `id` bigint unsigned NOT NULL auto_increment COMMENT 'id',
...
UPDATE `cloud`.`networks` n
JOIN `cloud`.`network_offerings` o
  ON o.id = n.network_offering_id
LEFT JOIN `cloud`.`ntwk_offering_service_map` m
  ON m.network_offering_id = o.id
SET n.`state` = 'Implemented'
WHERE n.`state` = 'Setup'
...
CALL `cloud_usage`.`IDEMPOTENT_CHANGE_COLUMN`('cloud_usage.quota_tariff','uuid','uuid','VARCHAR(40) NOT NULL');

-- Create column 'name'
CALL `cloud_usage`.`IDEMPOTENT_ADD_COLUMN`('cloud_usage.quota_tariff','name','TEXT COMMENT ''A name, defined by the user, to the tariff. This column will be used as identifier along the tariff updates.''');
UPDATE `cloud_usage`.`quota_tariff`
SET `name` = CASE
               WHEN `effective_on` <= NOW()
                 THEN COALESCE(`usage_name`, CONCAT('tariff-', `id`))
               ELSE CONCAT(COALESCE(`usage_name`, CONCAT('tariff-', `id`)), '-', `id`)
             END
WHERE `name` IS NULL OR TRIM(`name`) = '';
CALL `cloud_usage`.`IDEMPOTENT_CHANGE_COLUMN`('cloud_usage.quota_tariff','name','name','TEXT NOT NULL');

-- Create column 'description'
CALL `cloud_usage`.`IDEMPOTENT_ADD_COLUMN`('cloud_usage.quota_tariff','description','TEXT COMMENT ''The description of the tariff.''');
-- Create column 'activation_rule'
CALL `cloud_usage`.`IDEMPOTENT_ADD_COLUMN`('cloud_usage.quota_tariff','activation_rule','TEXT COMMENT ''JS expression that defines when the tariff should be activated.''');
-- Create column 'removed'
CALL `cloud_usage`.`IDEMPOTENT_ADD_COLUMN`('cloud_usage.quota_tariff','removed','DATETIME DEFAULT NULL');
-- Create column 'end_date'
CALL `cloud_usage`.`IDEMPOTENT_ADD_COLUMN`('cloud_usage.quota_tariff','end_date','DATETIME DEFAULT NULL COMMENT ''Defines the end date of the tariff.''');

-- Change usage unit to right unit
UPDATE `cloud_usage`.`quota_tariff`
SET `usage_unit` = CASE TRIM(`usage_unit`)
  WHEN 'Compute-Month' THEN 'Compute*Month'
  WHEN 'IP-Month'      THEN 'IP*Month'
  WHEN 'GB-Month'      THEN 'GB*Month'
  WHEN 'Policy-Month'  THEN 'Policy*Month'
  ELSE `usage_unit`
END
WHERE TRIM(`usage_unit`) IN ('Compute-Month','IP-Month','GB-Month','Policy-Month');
...
INSERT INTO `cloud`.`role_permissions` (uuid, role_id, rule, permission, sort_order)
SELECT UUID(), r.id, 'quotaStatement', 'ALLOW',
       (SELECT COALESCE(MAX(rp.sort_order),0) FROM `cloud`.`role_permissions` rp WHERE rp.role_id = r.id) - 1
FROM `cloud`.`roles` r
WHERE r.name = 'Read-Only User - Default'
  AND NOT EXISTS (
      SELECT 1 FROM `cloud`.`role_permissions` x
      WHERE x.role_id = r.id AND x.rule = 'quotaStatement'
  );

INSERT INTO `cloud`.`role_permissions` (uuid, role_id, rule, permission, sort_order)
SELECT UUID(), r.id, 'quotaBalance', 'ALLOW',
       (SELECT COALESCE(MAX(rp.sort_order),0) FROM `cloud`.`role_permissions` rp WHERE rp.role_id = r.id) - 2
FROM `cloud`.`roles` r
WHERE r.name = 'Read-Only User - Default'
  AND NOT EXISTS (
      SELECT 1 FROM `cloud`.`role_permissions` x
      WHERE x.role_id = r.id AND x.rule = 'quotaBalance'
  );
...
FROM `cloud`.`roles` r
WHERE r.role_type = 'DomainAdmin'
  AND NOT EXISTS (
        SELECT 1
          FROM `cloud`.`role_permissions` x
         WHERE x.role_id = r.id
           AND x.rule = 'isAccountAllowedToCreateOfferingsWithTags'
      );
...
CREATE TABLE IF NOT EXISTS `cloud`.`configuration_group` (
...
CREATE TABLE IF NOT EXISTS `cloud`.`configuration_subgroup` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
...
INSERT IGNORE INTO `cloud`.`configuration_group` (`name`, `description`, `precedence`) VALUES ('Miscellaneous', 'Miscellaneous configuration', 999);
INSERT IGNORE INTO `cloud`.`configuration_group` (`name`, `description`, `precedence`) VALUES ('Access', 'Identity and Access management configuration', 1);
INSERT IGNORE INTO `cloud`.`configuration_group` (`name`, `description`, `precedence`) VALUES ('Compute', 'Compute configuration', 2);
INSERT IGNORE INTO `cloud`.`configuration_group` (`name`, `description`, `precedence`) VALUES ('Storage', 'Storage configuration', 3);
INSERT IGNORE INTO `cloud`.`configuration_group` (`name`, `description`, `precedence`) VALUES ('Network', 'Network configuration', 4);
INSERT IGNORE INTO `cloud`.`configuration_group` (`name`, `description`, `precedence`) VALUES ('Hypervisor', 'Hypervisor specific configuration', 5);
INSERT IGNORE INTO `cloud`.`configuration_group` (`name`, `description`, `precedence`) VALUES ('Management Server', 'Management Server configuration', 6);
INSERT IGNORE INTO `cloud`.`configuration_group` (`name`, `description`, `precedence`) VALUES ('System VMs', 'System VMs related configuration', 7);
INSERT IGNORE INTO `cloud`.`configuration_group` (`name`, `description`, `precedence`) VALUES ('Infrastructure', 'Infrastructure configuration', 8);

INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Others', NULL, 999, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Miscellaneous'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Account', NULL, 1, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Access'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Domain', NULL, 2, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Access'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Project', NULL, 3, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Access'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('LDAP', NULL, 4, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Access'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('SAML', 'saml2', 5, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Access'));

INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Virtual Machine', 'vm,instance,cpu,ssh,affinity', 1, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Compute'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Kubernetes', 'kubernetes', 2, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Compute'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('High Availability', 'ha', 3, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Compute'));

INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Images', 'template,iso', 1, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Storage'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Volume', 'disk,diskoffering', 2, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Storage'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Snapshot', NULL, 3, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Storage'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('VM Snapshot', 'vmsnapshot', 4, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Storage'));

INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Network', 'firewall,vlan,dns,globodns,ipaddress,cidr', 1, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Network'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('DHCP', 'externaldhcp', 2, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Network'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('VPC', NULL, 3, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Network'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('LoadBalancer', 'lb,gslb', 4, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Network'));

INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('API', NULL, 1, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Management Server'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Alerts', 'alert', 2, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Management Server'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Events', 'event', 3, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Management Server'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Security', 'secure,password,authenticators', 4, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Management Server'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Usage', NULL, 5, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Management Server'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Limits', 'capacity,delay,interval,workers', 6, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Management Server'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Jobs', 'job', 7, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Management Server'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Agent', NULL, 8, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Management Server'));

INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Hypervisor', 'host', 1, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Hypervisor'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('KVM', 'libvirt', 2, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Hypervisor'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('VMware', 'vcenter', 3, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Hypervisor'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('XenServer', 'xen,xapi,XCP', 4, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Hypervisor'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('OVM', 'ovm3', 5, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Hypervisor'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Baremetal', NULL, 6, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Hypervisor'));

INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('ConsoleProxyVM', 'cpvm,consoleproxy,novnc', 1, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'System VMs'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('SecStorageVM', 'ssvm,secondary', 2, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'System VMs'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('VirtualRouter', 'vr,router,vrouter', 3, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'System VMs'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Diagnostics', NULL, 4, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'System VMs'));

INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Primary Storage', 'storage,pool,primary', 1, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Infrastructure'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Secondary Storage', 'image,secstorage', 2, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Infrastructure'));

INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Backup & Recovery', 'backup,recovery,veeam', 1, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Miscellaneous'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Certificate Authority', 'CA', 2, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Miscellaneous'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Quota', NULL, 3, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Miscellaneous'));
INSERT IGNORE INTO `cloud`.`configuration_subgroup` (`name`, `keywords`, `precedence`, `group_id`) VALUES ('Cloudian', NULL, 4, (SELECT id FROM `cloud`.`configuration_group` WHERE `name` = 'Miscellaneous'));
...
INSERT IGNORE INTO `cloud`.`role_permissions` (`uuid`, `role_id`, `rule`, `permission`) VALUES (UUID(), 2, 'assignVolume', 'ALLOW');
INSERT IGNORE INTO `cloud`.`role_permissions` (`uuid`, `role_id`, `rule`, `permission`) VALUES (UUID(), 3, 'assignVolume', 'ALLOW');

-- Increases the precision of the column `quota_used` from 15 to 20, keeping the scale of 8.

CALL `cloud_usage`.`IDEMPOTENT_CHANGE_COLUMN`(
  'cloud_usage.quota_usage',
  'quota_used',
  'quota_used',
  'DECIMAL(20,8) UNSIGNED NOT NULL'
);

CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.user','is_user_2fa_enabled','TINYINT NOT NULL DEFAULT 0');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.user','key_for_2fa','VARCHAR(255) DEFAULT NULL');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.user','user_2fa_provider','VARCHAR(255) DEFAULT NULL');
