...
-- 예시: 하나만 보여줌. 나머지 컬럼도 동일하게 IDEMPOTENT_ADD_COLUMN으로 교체
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','uuid','VARCHAR(40) UNIQUE DEFAULT NULL');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','name','VARCHAR(255) NOT NULL');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','display_text','VARCHAR(4096) DEFAULT NULL');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','unique_name','VARCHAR(32) DEFAULT NULL COMMENT ''unique name for system offerings''');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','customized','TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT ''0 implies not customized by default''');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','created','DATETIME DEFAULT NULL COMMENT ''date when service offering was created''');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','removed','DATETIME DEFAULT NULL COMMENT ''date when service offering was removed''');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','state','CHAR(40) NOT NULL DEFAULT ''Active'' COMMENT ''state of service offering either Active or Inactive''');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','disk_offering_id','BIGINT UNSIGNED');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','system_use','TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT ''is this offering for system used only''');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.service_offering','disk_offering_strictness','TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT ''strict binding with disk offering or not''');
...
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.disk_offering','disk_size_strictness', 'TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT ''To allow or disallow the resize operation on the disks created from this offering''');
CALL `cloud`.`IDEMPOTENT_ADD_COLUMN`('cloud.disk_offering','compute_only', 'TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT ''when set to 1, disk offering has one to one binding with service offering''');
...
CREATE TABLE IF NOT EXISTS `cloud`.`network_permissions` (
...
CREATE TABLE IF NOT EXISTS `cloud`.`vm_stats` (
...
CREATE TABLE IF NOT EXISTS `cloud`.`dc_ip6_guest_prefix` (
  `id` bigint unsigned NOT NULL auto_increment COMMENT 'id',
...
CREATE TABLE IF NOT EXISTS `cloud`.`ip6_guest_prefix_subnet_network_map` (
  `id` bigint unsigned NOT NULL auto_increment COMMENT 'id',
  `uuid` varchar(40) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `cloud`.`mshost_status` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'id',
...
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (2, 'Debian GNU/Linux 11 (64-bit)', 'XenServer', '8.2.1', 'Debian Bullseye 11');
CALL `cloud`.`ADD_GUEST_OS_AND_HYPERVISOR_MAPPING` (2, 'Debian GNU/Linux 11 (32-bit)', 'XenServer', '8.2.1', 'Debian Bullseye 11');