...
public final class ProcessRunner {
    ...
    public ProcessResult executeCommands(final List<String> commands, final Duration timeOut) {
        ...
        try {
            ...
            logger.debug(String.format("Process standard output for command : [%s].", stdOutput));
            logger.debug(String.format("Process standard error output command : [%s].", stdError));
        } catch (IOException | InterruptedException e) {
            logger.error(String.format("Exception caught error running command .", oneLineCommand), e);
            stdError = e.getMessage();
        }
